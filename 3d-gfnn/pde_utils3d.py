...
def get_u_evaluation_func_3d(greens_function: Callable[[Tuple[float, float, float], Tuple[float, float, float]], float], source_term: torch.Tensor, quadrature_mesh: torch.Tensor, chebyshev: bool = False):
    '''
    Returns the function to evaluate the integration of the greens function.
    '''
    def evaluate_u_discrete(coordinate, domain: Tuple[float, float, float, float, float, float], coordinate_filter_radius = 1e-5):
        ...
        area = (domain[1]-domain[0])*(domain[3]-domain[2])*(domain[5]-domain[4])
...
def evaluate_model_3d(model, f_values, f_meshes, coordinates, f_inds, domain):
    '''
    ...
    for i, coordinate in enumerate(coordinates):
        ind = f_inds[i]
        evaluation_func = get_u_evaluation_func_3d(greens_function=model, source_term=f_values[ind], quadrature_mesh=f_meshes[ind])
