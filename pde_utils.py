...
def get_u_evaluation_func(greens_function: Callable[[Tuple[float, float], Tuple[float, float]], float], source_term: Callable[[Tuple[float, float], Tuple[float, float]], float], integrate_bool: bool = False):
    '''
    ...
    def evaluate_u_discrete(coordinate, domain: Tuple[float, float, float, float], coordinate_filter_radius = 1e-5):
        ...
        pred = torch.sum(greens_function(filtered_mesh,torch.zeros_like(filtered_mesh)+torch.tensor(coordinate)) * source_term(filtered_mesh[:,0],filtered_mesh[:,1]))
        return pred
    if integrate_bool:
...
def evaluate_model(model, source_term, coordinates, domain):
    '''
