...
from pde_utils3d import evaluate_model_3d
...
class CustomDataPredLoss3d(object):
    ...
    def __call__(self, greens_function_approx, f_values, f_meshes, coordinates, domain, f_inds, u, *args, **kwargs):
        u_pred = evaluate_model_3d(model=greens_function_approx, f_values=f_values, f_meshes=f_meshes, f_inds=f_inds, coordinates=coordinates, domain=domain)
        diff = torch.nn.functional.mse_loss(u_pred, u)
        plot_multiple_points_3d([coordinates, coordinates], values_list=[u_pred, u], title_list=["Predicted Values", "Ground Truth"])
        return diff
