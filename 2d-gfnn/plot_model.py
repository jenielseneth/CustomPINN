...
from data_generation_utils import sample_points, get_interior_boundary_idx
from pde_utils import bnd_eval_gf_integral
from expr_generation_utils import expr_to_func, func_input_wrapper
from training_utils import UpdatedMultiDatasetWrapper
...
l_weights_line = find_line_with_keyword(file_path=model_dir + "main_info.txt", keyword="Learn Quadrature Weights", index=16)
...
uniform_mesh = sample_points(domain, num_points=(50,50), mesh_type="uniform")
...
weights = fetch_quadrature_weights(domain=domain, num_points=(f_mesh_quadrature_points[0], f_mesh_quadrature_points[1]), f_mesh_type=f_mesh_type_txt) 
...
for i in range(num_data):
    ...
    test_mesh = test_data[slice(*(test_data.sub_lengths[i]))]["crd"]
    ...
    if l_weights:
        ...
        col, bnd = get_interior_boundary_idx(domain, chebyshev_mesh)
    ...
    gt_test_mesh = test_data[slice(*(test_data.sub_lengths[i]))]["u_vals"]
    ...
    plot_multiple_points(points_list=[uniform_mesh, uniform_mesh, uniform_mesh, uniform_mesh, test_mesh, test_mesh], 
                        ...
                        main_title="Model Cheb. Interpolation prediction - Total Loss Term: " + str(torch.nn.functional.mse_loss(u_uniform, gt_uniform).item()),
                        save_dir=figure_dir, save_name=str(i), show=False)
    print("largest loss for data point", i, ":", torch.max(torch.nn.functional.mse_loss(u_uniform, gt_uniform, reduction="none")).item())
    print("Maximum value at predicted u(x) on uniform:", torch.max(torch.abs(u_uniform)).item())
    print("Maximum value at gt u(x) on uniform:", torch.max(torch.abs(gt_uniform)).item())
