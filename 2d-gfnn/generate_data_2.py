...
from expr_generation_utils import expr_to_func, func_input_wrapper, generate_u_expr, get_diffusion_term_a_expr, get_f_expr
...
if __name__ == "__main__":  
    ...
    u_exprs = generate_u_expr(n_expr=10)
    ...
    plot_uniform_mesh = sample_uniform_mesh_points((0, 5, 0, 5), (20, 20))
    ...
    for i, u_expr in enumerate(u_exprs):
        ...
        ##Generate Points
        generate_points(domain=domain, num_points=training_mesh_size, qudrature_num_points=qudrature_num_points, u_gt=u_changed[i], f_func=f_changed[i], u_gt_expr=function_str, f_func_expr=source_term_str, chebyshev=True, dir=subdir, training_data=True)
        generate_points(domain=domain, num_points=(400,), qudrature_num_points=qudrature_num_points, u_gt=u_changed[i], f_func=f_changed[i], u_gt_expr=function_str, f_func_expr=source_term_str, chebyshev=False, dir=subdir, training_data=False)
